...
def math_work():
    ...
    def m_done():
        ...
        c.image=victory_mp
        restartBut=Button(mwork,text='Go back into battle!', command=lambda: app_reset(app_root=mwork,where_from='math_work()'))
    ...
    def m_execute(value,math_sign_init):
        ...
        doneBut=Button(mwork,text='Mission Complete.',command=m_done).place(x=200,y=350)
    ...
    status=Label(mwork, text='The map in the background is marked with the top 20 loot locations.', bd=2, relief=SUNKEN, anchor=E)
...
def spelling_work():
    ...
    def s_done():
        ...
        c.image=victory_sp
        restartBut=Button(swork,text='Go back into battle!', command=lambda: app_reset(app_root=swork,where_from='spelling_work()'))
    ...
    def s_execute(option):
        ...
        doneBut=Button(swork,text='Mission Complete.',command=s_done).place(relx=0.5,y=600, anchor=CENTER)
    ...
    status= Label(swork, text='The map in the background has the locations of the bunkers, with passwords for those that need one!', bd=2, relief=SUNKEN, anchor=E)
